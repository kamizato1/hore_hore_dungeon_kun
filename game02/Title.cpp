...
Title::Title()
{
	...
	decision_se = LoadSoundMem("bgm/編集して使用したやつ/Click (mp3cut.net).mp3");
...
void Title::Update(Key* key)
{
	...
	if (CheckSoundMem(bgm) != 1) {   //SEが流れていなかったら再生
		ChangeVolumeSoundMem(255 * 80 / 100, bgm); //SE音量調整 255最大音量から80%再生
	...
	if (credit == false && help == false)
	{
		...
		{
			help = true;
			ChangeVolumeSoundMem(255 * 80 / 100, decision_se); //SE音量調整 255最大音量から80%再生
		...
		{
			ChangeVolumeSoundMem(255 * 80 / 100, decision_se); //SE音量調整 255最大音量から80%再生
			PlaySoundMem(decision_se, DX_PLAYTYPE_NORMAL, TRUE); //SE再生
	...
	if (help)
	{
		...
		if (++input_time > INPUT_ACCEPTANCE_TIME)
		{
			if (key->GetStickAngle(L).x > 0)
			{
				...
				ChangeVolumeSoundMem(255 * 80 / 100, move_se); //SE音量調整 255最大音量から80%再生
