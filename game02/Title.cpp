...
void Title::Update(Key* key)
{
	...
	if (CheckSoundMem(bgm) != 1) {   //SEが流れていなかったら再生
		ChangeVolumeSoundMem(255, bgm); //SE音量調整 255最大音量から80%再生
	...
	if (credit == false && help == false)
	{
		...
		{
			help = true;
			ChangeVolumeSoundMem(255, decision_se); //SE音量調整 255最大音量から80%再生
	...
	if (help)
	{
		...
		if (++input_time > INPUT_ACCEPTANCE_TIME)
		{
			if (key->GetStickAngle(L).x > 0)
			{
				...
				ChangeVolumeSoundMem(255, move_se); //SE音量調整 255最大音量から80%再生
